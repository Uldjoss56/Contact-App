...
import 'package:contact/ui/contact/contact_edit_page.dart';
...
class ContactTile extends StatelessWidget {
      ...
      startActionPane: ActionPane(
        ...
        children: [
          ...
          SlidableAction(
            ...
            icon: Icons.email,
            label: 'Email',
            onPressed: (context) => _writeEmail(context, displayContact.email),
          ),
        ],
      ),
  ...
  Future _writeEmail(BuildContext context, String emailAddress) async {
    ...
    if (await url_launcher.canLaunchUrl(url as Uri)) {
      ...
      const snackbar = SnackBar(content: Text('Can\'t write a message'));
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(snackbar);
    }
  }
  ...
  Hero buildCircleAvatar(Contact displayContact) {
    return Hero(
      ...
      child: CircleAvatar(child: _buildCircleAvatarContent(displayContact)),
    );
  }
  ...
  Widget _buildCircleAvatarContent(Contact displayContact) {
    if (displayContact.imageFile == null) {
      ...
      return ClipOval(
        child: AspectRatio(
          aspectRatio: 1,
          child: Image.file(displayContact.imageFile!, fit: BoxFit.cover),
        ),
      );
    }
  }
